...
namespace iar_astar_planner
{
    ...
    bool NavFn::propAstar(int cycles)
    {
        ...
        */
        float dx = goal_[0] - start_[0];
        float dy = goal_[1] - start_[1];

        float heuristic = std::sqrt(dx * dx + dy * dy);

        potentThresh_ = COST_OBS + heuristic;
    ...
    inline void NavFn::updateCell(int n)
    {
        ...
        {
            float l, r, u, d, ul, ur, dl, dr; //changed from lrtb, tl, tr, bl, br
            ...
            */
            ...
            u = potarr_[n - nx_] + costarr_[n];
            d = potarr_[n + nx_] + costarr_[n];        
            ...
            */
            ...
            //tutorial copied
            ul = potarr_[n - nx_ - 1] + SQUAREROOT2 * costarr_[n];
            ur = potarr_[n - nx_ + 1] + SQUAREROOT2 * costarr_[n];
            dl = potarr_[n + nx_ - 1] + SQUAREROOT2 * costarr_[n];
            dr = potarr_[n + nx_ + 1] + SQUAREROOT2 * costarr_[n];
            ...
            */
            ...
            pot = std::min({l, r, u, d, ul, ur, dl, dr});
                ...
                less than the existing saved potential estimate */ 
            {
                ...
                */
                  float dx = goal_[0] - (n % nx_);
                  float dy = goal_[1] - (n / nx_);

                  float hf = std::sqrt(dx * dx + dy * dy);
                ...
                if (pot < potentThresh_)
                {
                    ...
                    */
                    ...
                    //tutorial copied
                    if (potarr_[n - 1] > pot + costarr_[n - 1]) {push_next(n - 1);}
                    if (potarr_[n + 1] > pot + costarr_[n + 1]) {push_next(n + 1);}
                    if (potarr_[n - nx_] > pot + costarr_[n - nx_]) {push_next(n - nx_);}
                    if (potarr_[n + nx_] > pot + costarr_[n + nx_]) {push_next(n + nx_);}
                    ...
                    */
                    if (potarr_[n - nx_ - 1] > pot + SQUAREROOT2 * costarr_[n - nx_ - 1]) {push_next(n - nx_ - 1);}
                    if (potarr_[n - nx_ + 1] > pot + SQUAREROOT2 * costarr_[n - nx_ + 1]) {push_next(n - nx_ + 1);}
                    if (potarr_[n + nx_ - 1] > pot + SQUAREROOT2 * costarr_[n + nx_ - 1]) {push_next(n + nx_ - 1);}
                    if (potarr_[n + nx_ + 1] > pot + SQUAREROOT2 * costarr_[n + nx_ + 1]) {push_next(n + nx_ + 1);}
                    ...
                    */
                    ...
                    if (potarr_[n - 1] > pot + costarr_[n - 1]) {push_over(n - 1);}
                    if (potarr_[n + 1] > pot + costarr_[n + 1]) {push_over(n + 1);}
                    if (potarr_[n - nx_] > pot + costarr_[n - nx_]) {push_over(n - nx_);}
                    if (potarr_[n + nx_] > pot + costarr_[n + nx_]) {push_over(n + nx_);}
                    ...
                    /* TODO TASK - MILESTONE # 3.9
                        ...
                        Please note the difference in the formula to that of above task.
                    */
                    if (potarr_[n - nx_ - 1] > pot + SQUAREROOT2 * costarr_[n - nx_ - 1]) {push_over(n - nx_ - 1);}
                    if (potarr_[n - nx_ + 1] > pot + SQUAREROOT2 * costarr_[n - nx_ + 1]) {push_over(n - nx_ + 1);}
                    if (potarr_[n + nx_ - 1] > pot + SQUAREROOT2 * costarr_[n + nx_ - 1]) {push_over(n + nx_ - 1);}
                    if (potarr_[n + nx_ + 1] > pot + SQUAREROOT2 * costarr_[n + nx_ + 1]) {push_over(n + nx_ + 1);}
