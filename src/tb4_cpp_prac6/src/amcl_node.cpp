...
#include "iar_amcl/iar_pf_kdtree.hpp"
...
AmclNode::initParticleFilter()
{
  ...
  // pf_ = pf_alloc(
  //   min_particles_, max_particles_, alpha_slow_, alpha_fast_,
  //   (pf_init_model_fn_t)AmclNode::uniformPoseGenerator, reinterpret_cast<void *>(map_));
  pf_->pop_err = pf_err_;
  pf_->pop_z = pf_z_;
...
AmclNode::pf_resample(pf_t * pf){
  ...
  */
  ...
  for (int i = 0; i < old_particle_set->sample_count; i++) {
    sample_in_old_set = old_particle_set->samples + i;
    // running sum where each entry = previous entry + this particle's weight
    cd_old_set[i+1] = cd_old_set[i] + sample_in_old_set->weight;
  }
  ...
  */
  new_particle_set->sample_count=0;
  ...
  */
  double w_diff = 1.0-pf->w_fast / pf->w_slow; //probability for random particle to recover being kidnapped
  if (w_diff<0.0) {
    w_diff = 0.0; //prevent negative probability
  }
  ...
  while(new_particle_set->sample_count < pf->max_samples)
  {
    ...
    if(drand48() < w_diff){
      ...
      */
      sample_in_new_set = new_particle_set->samples + new_particle_set->sample_count; //place random particle on map
      ...
      */
      //survival of fittest resampling with highest weights
      double r = drand48();
      int i =0;
      while ((i<old_particle_set->sample_count) && !((cd_old_set[i]<=r) && (r<cd_old_set[i+1]))) {
        i++;
      }
      sample_in_old_set = old_particle_set->samples+i;
    ...
    */
    //code
    int M;
    int k = new_particle_set->kdtree->leaf_count; //confidence level for closer particles together
    ...
    if (k <= 1) {
      ...
      double c = sqrt(b) * pf_z_;
      M = static_cast<int>(ceil(
        ((k - 1) / (2.0 * pf_err_)) * pow(a - b + c, 3)));
  ...
  */
  //from tut part 3
  for (int i = 0; i < new_particle_set->sample_count; i++) {
    sample_in_new_set = new_particle_set->samples + i;
    sample_in_new_set->weight /= total_weight;
  }
