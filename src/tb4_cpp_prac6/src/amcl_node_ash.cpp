...
AmclNode::pf_resample(pf_t * pf){
  ...
  */
  ...
  while(new_particle_set->sample_count < pf->max_samples)
  {
    ...
    if(drand48() < w_diff){
      ...
      CHECK THIS WITH TIM, HIS IS DIFFERENT  */

      sample_in_new_set->pose = (pf->random_pose_fn)(reinterpret_cast<void *>(map_));
    } 
    else {
      /* TODO TASK - MILESTONE # 4.2
        Generate samples based on weight 
      DONT KNOW IF THIS WORKS, IF THE CODE DOESNT WORK ITS PROBABLY THIS    */
      double random_weight = drand48() * cd_old_set[old_particle_set->sample_count];
      int sample_index = 0;
      while (sample_index < old_particle_set->sample_count - 1 &&
        random_weight > cd_old_set[sample_index + 1])
      {
        sample_index++;
      }
      sample_in_new_set->pose = old_particle_set->samples[sample_index].pose;
    }
    /* TODO TASK - MILESTONE # 5
      Allocate weights to new particles, and calculate total weights
    */
    sample_in_new_set->weight = 1.0;
    new_particle_set->total_weight += sample_in_new_set->weight;
    ...
    */
    ...
    if (k <= 1) {
      ...
      double left = (k - 1.0) / (2.0 * pf->pop_err_);
      double middle = 2.0 / (9.0 * (k - 1.0));
      double right = sqrt(middle) * iar_amcl::AmclNode->pf_z_;

      M = static_cast<int>(std::max(pf->min_samples, left * std::pow(1 - middle + right, 3)));
  ...
  */
  for (int i = 0; i < new_particle_set->sample_count; i++) {
    ...
    sample_in_new_set->weight /= new_particle_set->total_weight;
  }
