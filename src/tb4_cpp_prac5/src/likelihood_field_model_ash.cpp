...
namespace iar_amcl
{
    ...
    LikelihoodFieldModel::sensorFunction(nav2_amcl::LaserData * data, pf_sample_set_t * set)
    {
        ...
        for (j = 0; j < set->sample_count; j++) {
               ...
               pose = pf_vector_coord_add(self->laser_pose_, pose_robot);
                ...
                for (i = 0; i < data->range_count; i += step) {
                    ...
                    */
                    ...
                    if (isnan(obs_range) || isinf(obs_range)) {
                        continue;
                    }
                    ...
                    */
                        ...
                        double beam_angle = pose.v[2] + obs_bearing;
                        double hit_x = pose.v[0] + obs_range * cos(beam_angle);
                        double hit_y = pose.v[1] + obs_range * sin(beam_angle);
                        //int g_x = MAP_GXWX(self->map_, hit_x);
                        //int g_y = MAP_GYWY(self->map_, hit_y);

                        int m_x = static_cast<int>(MAP_GXWX(self->map_, hit_x));
                        int m_y = static_cast<int>(MAP_GYWY(self->map_, hit_y));


                        if (MAP_VALID(self->map_, m_x, m_y)) {
                            int i = MAP_INDEX(self->map_, m_x, m_y);
