    ...
    LikelihoodFieldModel::sensorFunction(nav2_amcl::LaserData * data, pf_sample_set_t * set)
    {
        ...
        for (j = 0; j < set->sample_count; j++) {
                ...
                */
                ...
                pose_robot = pf_vector_coord_add(self->laser_pose_, pose_robot); //changed form pose_robot
                p = 1.0;
                ...
                for (i = 0; i < data->range_count; i += step) {
                    ...
                    */
                      ...
                      hit_x= pose_robot.v[0] + obs_range*cos(pose_robot.v[2]+obs_bearing);
                      hit_y= pose_robot.v[1] + obs_range*sin(pose_robot.v[2]+obs_bearing);
                    
                      //if {xhit , yhit } is a valid point in map then Retrieve distance dist between point {xhit , yhit } and its nearest obstacle point in the map m.
                    
                    
                    //============ MAP_VALID used wrong i think
                      if (MAP_VALID(hit_x, hit_y)) { //MAPVALID
                        dist= map_dist(hit_x, hit_y); //map_dist doesnt exist
                      //========== error
                      } else {
